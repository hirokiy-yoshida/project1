  ...
  try {
    ...
    const query = `
      ...
      WHERE
        ...
        AND Omotenasi__c = true
      ORDER BY 
        Sort__c ASC NULLS LAST,
        Name ASC
    `.trim();

    console.log('Executing foods query:', query);
    ...
    console.log('Foods response:', JSON.stringify(response.data, null, 2));
    ...
    const foods = response.data.records.map((record: any) => ({
      id: record.Id,
      name: record.Name,
      sort: record.Sort__c
    }));
    ...
    console.error('Error in /api/foods:', errorInfo);
    return NextResponse.json(
